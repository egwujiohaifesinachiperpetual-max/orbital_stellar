...
async function createPostgresStore(): Promise<any> {
  ...
  return {
    async getCursor() {
      const v = await store.get("test-stream");
      return v ?? undefined;
    },
    async saveCursor(c: string) {
      await store.set("test-stream", c);
    },
    async cleanup() {
      try {
        await pool.query("DELETE FROM cursor_store WHERE stream_key = $1", ["test-stream"]);
      } catch {}
      await pool.end();
    },
  };
...
describe("Integration: cursor resume across stores", () => {
  ...
  stores.push({
    name: "file",
    factory: async () => createFileStore(await fs.mkdtemp(path.join(os.tmpdir(), "cursor-"))),
  });
  ...
  for (const storeDesc of stores) {
    ...
    testFn(
      `store=${storeDesc.name} should resume cursor across restart (kill-restart invariant)`,
      async () => {
        const factoryResult = await (typeof storeDesc.factory === "function"
          ? storeDesc.factory()
          : storeDesc.factory);
